...
public class SearchShardsResponseBwcIT extends ESRestTestCase {
    ...
    public void testSkippedShardsPreservedAcrossVersions() throws Exception {
        assumeTrue("requires multi-cluster CCS setup",
            "multi_cluster".equals(System.getProperty("tests.rest.suite")));
        ...
        try (RestClient remoteClient = newRemoteClient()) {
            ...
            for (int day = 1; day <= 5; day++) {
                ...
                createIndex(remoteClient, index, Settings.builder()
        ...
        assertThat("should find the 3 documents from day 1",
            (int) result.evaluate("hits.total.value"), equalTo(3));
        assertThat("all 5 shards should be accounted for in the total",
            (int) result.evaluate("_shards.total"), equalTo(5));
        assertThat("4 non-matching shards should be reported as skipped",
            (int) result.evaluate("_shards.skipped"), equalTo(4));
        assertThat("all 5 shards should be successful",
            (int) result.evaluate("_shards.successful"), equalTo(5));
        assertThat("no shards should fail",
            (int) result.evaluate("_shards.failed"), equalTo(0));
    }
}
